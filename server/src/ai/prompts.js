...
export function buildTrainingGradePrompt({ intent_ko, focus_ko, reference_en, answer_text, attempt_no }) {
  return [
    ...
    'On a failed first attempt, hint_ko must have exactly two lines joined by a single newline character, in this exact format:',
    '"구조: <Korean sentence structure template, e.g. 주어 + have been + 동사ing + for 기간>"',
    '"핵심 표현: <one English chunk of at most 3 words>"',
    'hint_ko must never quote more than 3 consecutive words from the reference example.',
