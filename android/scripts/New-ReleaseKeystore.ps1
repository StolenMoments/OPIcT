...
$keystorePath = Join-Path $keystoreDirectory 'opict-release.jks'
...
try {
    ...
    $keytoolOutput = & $keytool `
        ...
        -keypass $keyPassword `
        -dname 'CN=OPIcT, OU=Personal, O=mygreed, L=Seoul, ST=Seoul, C=KR' 2>&1
    ...
    $properties = @(
        'storeFile=keystore/opict-release.jks'
