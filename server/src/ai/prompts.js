...
export function buildEvalPrompt(questionText, transcript) {
  return [
    ...
    'Respond with ONLY a JSON object, no prose, matching exactly:',
    '{"summary_ko":"과제를 충실히 수행했습니다.","strengths_ko":["경험을 구체적으로 설명했습니다."],"improvements_ko":["시제를 일관되게 사용하세요."],"recommended_expressions":[{"text":"clear my head","note_ko":"머리를 식히다"}],"corrected_answer":"I went to the park yesterday."}',
...
export function buildEvalRepairPrompt(questionText, transcript, validationError, failedRaw) {
  return repairPrompt({
    ...
    contract: '{"summary_ko":"과제를 충실히 수행했습니다.","strengths_ko":["경험을 구체적으로 설명했습니다."],"improvements_ko":["시제를 일관되게 사용하세요."],"recommended_expressions":[{"text":"clear my head","note_ko":"머리를 식히다"}],"corrected_answer":"I went to the park yesterday."}',
    validationError,
    failedRaw,
    context: [
