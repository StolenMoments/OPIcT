...
`keystore/opict-release.jks`와 `keystore.properties`는 git에서 제외된다. 두 파일을
암호화된 별도 저장소에 함께 백업해야 한다. 하나라도 잃으면 기존에 설치한 앱을 같은
application ID로 업데이트할 수 없으며, 새 키로 서명한 APK는 기존 앱 제거 후에만
설치할 수 있다.

## 수동 확인

- 최초 실행에서 비밀번호 입력 화면이 표시되는지 확인한다.
- 로그인 후 연습, 교정, 노트, 기록, 설정과 로그아웃을 확인한다.
